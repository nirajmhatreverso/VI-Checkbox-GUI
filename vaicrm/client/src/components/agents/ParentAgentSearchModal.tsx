...
type FilterMode = "mobile" | "sapBpId" | "location" | "name";

interface Props {
  isOpen: boolean;
  onClose: () => void;
  onSelect: (agent: AgentApiItem) => void;
  // ✅ Added Prop to control Subscription vs Hardware mode
  isSubCollection?: "Y" | "N";
}
...
export default function ParentAgentSearchModal({
  isOpen,
  onClose,
  onSelect,
  ...
  const handleBpIdChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value.replace(/\D/g, "").slice(0, 10);
    setSapBpId(value);
  };

  const fetchAgents = async () => {
    const isIdProvided = (mode === "mobile" && mobile.trim()) || (mode === "sapBpId" && sapBpId.trim()) || (mode === "name" && name.trim());
    ...
    try {
      const payload: any = {
        type: "Agent",
        isSubCollection: isSubCollection, // ✅ Use the prop value here
      ...
      // Use generic API helper if agentApi is not available or modify accordingly
      ...
      const mappedResults: AgentApiItem[] = res.data.customerDetails.map((item: any) => {
        ...
        return {
          ...
          currency: relatedParties.find((r: any) => r.currency)?.currency || "",
          salesOrg: relatedParties.find((r: any) => r.salesOrg)?.salesOrg || user?.salesOrg || "",
          division: relatedParties.find((r: any) => r.division)?.division || "",
      ...
      toast({ title: "Search Failed", description: err.message || "No agents found", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-[95vw] sm:max-w-2xl rounded-lg">
        <DialogHeader>
          <DialogTitle><Badge variant="outline" className="bg-blue-50 text-blue-700">Parent Agent Search</Badge></DialogTitle>
          <DialogDescription>Select and assign SAP BP ID as Parent Agent</DialogDescription>
        </DialogHeader>
        ...
        <Tabs value={mode} onValueChange={(v) => setMode(v as FilterMode)} className="w-full">
          <TabsList className="grid grid-cols-4 mb-4">
            ...
            <TabsTrigger value="sapBpId">SAP BP ID</TabsTrigger>
          ...
          <TabsContent value="sapBpId">
            ...
            <Input placeholder="Enter BP ID..." value={sapBpId} onChange={handleBpIdChange} className={controlClasses} />
          </TabsContent>
  ...
  <Input 
    ...
    value={name} 
    onChange={(e) => setName(e.target.value.replace(/[^A-Za-z\s]/g, ""))} // Only letters and spaces
  ...
  />
        ...
        <Button className="w-full mt-2 h-8 text-xs bg-azam-blue hover:bg-blue-700" onClick={fetchAgents} disabled={loading}>
          {loading ? <><Loader2 className="h-3 w-3 animate-spin mr-2" /> Searching...</> : <><Search className="h-3 w-3 mr-2" /> Find Parent Agent</>}
        </Button>
