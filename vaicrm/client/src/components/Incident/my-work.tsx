...
export default function MyWork() {
  ...
  // Fetch Incidents Query - Using logged-in username
  const { data: incidentsData, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['my-work-incidents', user?.username],
    queryFn: async () => {
      const payload = {
        objId: null,
        title: null,
        ...
        incidentOwner2user: user?.username || "",
  ...
  // Handler for clicking on closed incident
  const handleClosedIncidentClick = (incidentNumber: string) => {
    toast({
      title: "Cannot Edit Closed Incident",
      description: `Incident ${incidentNumber} is closed and cannot be edited.`,
      variant: "destructive",
      duration: 3000,
    });
  };
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = `my_work_tickets_${user?.username}_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
          ...
          <CardHeader className="pb-3">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <CardDescription>
                  {user?.username
                    ? `Incidents assigned to or created by: ${user.username}`
  ...
  <div className="rounded-md border w-full">
    <div className="overflow-x-auto w-full">
      <div style={{ minWidth: '1600px' }}>
        <DataTable<Incident>
          title={`My Tickets (${filteredIncidents.length})`}
          icon={<AlertTriangle className="h-5 w-5 text-azam-blue" />}
          headerVariant="gradient"
          data={filteredIncidents}
          columns={columns}
          loading={isLoading}
          error={isError ? (error instanceof Error ? error.message : String(error)) : undefined}
          className="w-full"
        />
      </div>
    </div>
            ...
            {/* Summary Stats */}
            {user?.username && !isLoading && incidents.length > 0 && (
              <div className="flex items-center gap-4 text-xs text-gray-500 pt-2">
                ...
                <span>
                  Closed: <strong>{incidents.filter(i => i.status === 'CLOSED').length}</strong>
                </span>
              </div>
            )}
            ...
            {user?.username && !isLoading && incidents.length === 0 && (
              <div className="flex flex-col items-center justify-center py-12 text-gray-500">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  You don't have any incidents assigned to or created by you
                </p>
              </div>
            )}
          </CardContent>
