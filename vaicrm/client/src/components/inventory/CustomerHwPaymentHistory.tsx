...
import { FileText, Search, Info, SlidersHorizontal, X, Calendar as CalendarIcon, CheckCircle2, AlertTriangle } from "lucide-react";
...
export default function CustomerHwSaleHistory() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["hw-history-rows-customer", filters, loggedInUserBpId, isAdminUser],
    staleTime: 15_000,
  ...
  const columns: DataTableColumn<HistoryRow>[] = [
    ...
    { key: "totalAmount", label: "Total", sortable: true, render: (_v, r) => `${r.currency} ${Number(r.totalAmount).toLocaleString()}` },
    {
      key: "status",
      label: "Status",
      sortable: true,
    ...
    // CM Status column with curved bg and tooltip
    {
  ...
  const handleReset = () => {
    setFilters({ requestId: "", sapBpId: "", status: "ALL", fromDate: initialFrom, toDate: initialTo });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>View customer hardware sale requests and their status</CardDescription>
      </CardHeader>
      ...
      <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>{detailsRow?.requestId} • {detailsRow?.sapBpId}</DialogDescription>
          </DialogHeader>
          {detailsRow && (
              ...
              {detailsRow?.raw?.[0] && (
                <>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2 border p-2 rounded">
                    <div><div className="text-gray-500">SAP CA ID</div><div className="font-medium">{detailsRow.raw[0].sapCaId || "-"}</div></div>
                    <div><div className="text-gray-500">Module</div><div className="font-medium">{detailsRow.raw[0].module || "-"}</div></div>
                    <div><div className="text-gray-500">Plant Name</div><div className="font-medium">{detailsRow.raw[0].plantName || "-"}</div></div>
                    <div><div className="text-gray-500">SAP SO ID</div><div className="font-medium">{detailsRow.raw[0].sapSoId || "-"}</div></div>
                    <div className="col-span-full"><div className="text-gray-500">Remark</div><div className="font-medium">{detailsRow.raw[0].remark || "-"}</div></div>
                  </div>
...
function AdvancedFiltersComponent({ advFilters, setAdvFilters, setFilters, initialFrom, initialTo, isAdminUser }: {
  ...
  const addAdvFilter = (field: FilterFieldKey) => {
    if (field === "dateRange" && advFilters.some((f: AdvancedFilter) => f.field === "dateRange")) return;
  ...
  useEffect(() => {
    const nextFilters: HistoryFilters = { requestId: "", sapBpId: "", status: "ALL", fromDate: initialFrom, toDate: initialTo };
    advFilters.forEach((f: AdvancedFilter) => {
      switch (f.field) {
        ...
        case "status": nextFilters.status = f.value || "ALL"; break;
        case "dateRange":
          if (f.dateRange?.from) {
    ...
    <div className="space-y-3 border p-3 rounded-md">
      ...
      <div className="space-y-2">
        ...
        {advFilters.map((af: AdvancedFilter) => {
          ...
          const ops = fieldMeta.type === 'text' ? ["contains", "equals", "startsWith", "endsWith"] : ["equals"];
          return (
            <div key={af.id} className="grid grid-cols-12 gap-2 items-center">
              ...
              <div className="col-span-6">
                ...
                {fieldMeta.type === 'select' && af.field === 'status' && <Select value={af.value} onValueChange={v => setAdvFilters((p: AdvancedFilter[]) => p.map(x => x.id === af.id ? { ...x, value: v } : x))}><SelectTrigger className="h-7 text-xs"><SelectValue placeholder="Select status" /></SelectTrigger><SelectContent><SelectItem value="ALL">ALL</SelectItem><SelectItem value="INPROCESS">INPROCESS</SelectItem><SelectItem value="SUCCESS">SUCCESS</SelectItem><SelectItem value="REJECTED">REJECTED</SelectItem></SelectContent></Select>}
