...
export default function StockRequestDelivery() {
...
const shouldSendSapBpId = user?.isOtc === "Y" || user?.isMainPlant === "Y";
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["stock-delivery-queue", debouncedFilters,shouldSendSapBpId ? user?.sapBpId : null],
    queryFn: () => apiRequest('/inventory/history', 'POST', {
      requestId: debouncedFilters.requestId || null,
      fromPlantName: debouncedFilters.fromPlantName || null,
      material: debouncedFilters.material || null, // Pass to API
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: (res, vars) => {
      if (res && (res.status === "SUCCESS" || res.status === 200 || res.statusMessage?.includes("Success"))) {
        ...
        queryClient.invalidateQueries({ queryKey: ["stock-delivery-queue"] });
      } else {
        toast({
  ...
  const deliveryItems = useMemo((): StockItem[] => {
    const rawList = data?.data?.stockDetails || [];

    return rawList.filter((item: StockItem) => {
      // 1. Basic Status Check
      if (item.status !== "APPROVED" || item.isFileUpload !== "N" || item.approvedItemQty <= 0) {
        return false;
      }
  ...
  const handleReset = () => {
    setFilters({ requestId: "", fromPlantName: "", toPlantName: "", material: "", fromDate: initialFrom, toDate: initialTo });
  ...
  const columns: DataTableColumn<StockItem>[] = [
    ...
    { key: "fromPlantName", label: "From Plant" },
    { key: "toPlantName", label: "To Plant" },
    {
      key: "material",
      label: "Material",
      render: (_v, row) => (
  ...
  key: "itemQty",
  label: "Req Qty",
  render: (value, row) => String(value ?? row.itemQty ?? 0),
},
{
  key: "approvedItemQty",
  label: "Appr Qty",
  render: (value, row) => String(value ?? row.approvedItemQty ?? 0),
},
    {
      key: "status",
      label: "Status",
  ...
  const errorMessage = "Failed to load pending deliveries.";
      ...
      <CardHeader className="px-0 pt-0">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex-1">
            ...
            <CardDescription>Upload serial numbers for approved stock requests to complete delivery.</CardDescription>
          </div>
        ...
        {!useAdvanced && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-2 border p-2 rounded-md">
            <div>
              ...
              <Input value={filters.requestId} onChange={(e) => setFilters((f) => ({ ...f, requestId: e.target.value }))} placeholder="SR..." className="h-8 text-xs" />
            </div>
            <div>
              <LabelSmall>From Plant</LabelSmall>
              <Input value={filters.fromPlantName} onChange={(e) => setFilters((f) => ({ ...f, fromPlantName: e.target.value }))} placeholder="Plant Name..." className="h-8 text-xs" />
            </div>
          ...
          <DialogContent className="max-w-2xl">
            ...
            {detailsItem && (
              <div className="grid grid-cols-2 gap-4 text-sm mt-2">
                ...
                <div className="space-y-1">
  <div className="text-xs text-gray-500 uppercase">Requested Qty</div>
  <div className="font-medium">{detailsItem.itemQty ?? "-"}</div>
</div>

<div className="space-y-1">
  <div className="text-xs text-gray-500 uppercase">Approved Qty</div>
  <div className="font-medium">{detailsItem.approvedItemQty ?? "-"}</div>
</div>
                <div className="space-y-1">
...
function AdvancedFiltersComponent({
  ...
  useEffect(() => {
    const nextFilters: StockDeliveryFilters = { requestId: "", fromPlantName: "", toPlantName: "", material: "", fromDate: initialFrom, toDate: initialTo };
    advFilters.forEach((f) => {
      switch (f.field) {
        case "requestId": nextFilters.requestId = f.value || ""; break;
        case "material": nextFilters.material = f.value || ""; break; // Map material
