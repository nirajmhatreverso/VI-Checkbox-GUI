...
import AgentSubagentTransferHistory from "./AgentSubagentTransferHistory";
import AgentSubagentTransferDelivery from "./AgentSubagentTransferDelivery";
...
export default function AgentSubagentTransfer() {
    ...
    // ✅ Auto-fill "From Agent" for non-admin/non-mainplant users (agent users)
    useEffect(() => {
        if (!canSearchAgents && loggedInUserBpId) {
            const agentData = {
                agentName: user?.name || user?.username || "Agent",
    ...
    const handleMaterialSelect = useCallback((id: string, materialCode: string) => {
        ...
        if (!formData.fromPlant) {
            toast({ title: "Selection Missing", description: "Please select 'From Agent' before adding items.", variant: "destructive" });
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const res = await agentApi.subAgentStockTransfer(payload);
    ...
    // ✅ Handler for "From Agent" input click - only works for users who can search agents
    const handleFromAgentClick = () => {
        if (canSearchAgents) {
        ...
        <div className="p-4 sm:p-6 space-y-6">
            <div className="bg-gradient-to-r from-azam-blue to-blue-800 text-white p-4 rounded-lg">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-xl font-bold">Agent to Sub agent transfer</h1>
                        <p className="text-blue-100 text-[11px] md:text-xs leading-tight mt-0.5 sm:block">
                            Transfer stock from agent to sub-agent and track deliveries
                        </p>
                    </div>
                </div>
            </div>
            ...
            <Card className="p-0">
                <Tabs value={tab} onValueChange={setTab} className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                                    ...
                                    <div className="space-y-2 relative md:col-span-2">
                                        <div className="flex items-center justify-between">
                                            <Label className="text-xs">From Agent <span className="text-red-500">*</span></Label>
                                            {/* ✅ Only show filter button for users who can search agents */}
                                            {canSearchAgents && (
                                                <Button
                                                    type="button"
                                                    variant="ghost"
                                        ...
                                        <Input
                                            uiSize="sm"
                                            placeholder={canSearchAgents ? "Select Source Agent" : "Auto-filled from your account"}
                                            value={selectedFromAgent ? `${selectedFromAgent.agentName} (${selectedFromAgent.sapBpId})` : ""}
                                            readOnly
                                            onClick={handleFromAgentClick}
                                            className={cn(
                                                "bg-white",
                                                canSearchAgents
                                                    ? "cursor-pointer"
                                        ...
                                        />
                                        {/* ✅ Show helper text for non-admin/non-mainplant users */}

                                    </div>

                                    <div className="space-y-2 relative md:col-span-2">
                                        <div className="flex items-center justify-between">
                                            <Label className="text-xs">To Sub-agent <span className="text-red-500">*</span></Label>
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                                                        <div className="md:col-span-3">
                                                            ...
                                                            >
                                                                ...
                                                                <SelectContent>
                                                                    {materials
                                                                        .filter((m: any) => m.productPriceType === (formData.priceType === "KIT" ? "Agent Kit Price" : "Agent Individual Price"))
                                                                        .map((m: any, materialIdx: number) => (
                                                                            <SelectItem
                                                                                key={`material-${it.id}-${m.productId}-${materialIdx}`}
                                                                                value={m.productId}
                    ...
                    <TabsContent value="delivery" className="space-y-6">
                        <AgentSubagentTransferDelivery />
                    </TabsContent>

                    <TabsContent value="history" className="space-y-6">
                        <AgentSubagentTransferHistory />
                    </TabsContent>
                </Tabs>
            </Card>

            {/* ✅ Only render ParentAgentSearchModal for users who can search agents */}
            {canSearchAgents && (
