...
export default function StockTransferDelivery() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["stock-transfer-delivery-queue", debouncedFilters],
    queryFn: () => apiRequest('/inventory/history', 'POST', {
      ...
      material: debouncedFilters.material || null, // Passed to API
      fromDate: debouncedFilters.fromDate,
      toDate: debouncedFilters.toDate,
      status: "APPROVED",
      type: "ST" // Stock Transfer
    }),
    staleTime: 60_000,
  });
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: (res, vars) => {
      if (res && (res.status === "SUCCESS" || res.status === 200 || res.statusMessage?.includes("Success"))) {
        ...
        queryClient.invalidateQueries({ queryKey: ["stock-transfer-delivery-queue"] });
      } else {
        toast({
  ...
  const columns: DataTableColumn<StockItem>[] = [
    ...
    { key: "toPlantName", label: "To Plant" },
    {
      key: "material",
      label: "Material",
      // UPDATED: Show Item Name (itemType) & Material Code
      render: (_v, row) => (
      ...
      <CardHeader className="px-0 pt-0">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex-1">
            ...
            <CardDescription>Upload serial numbers for approved stock transfers to complete delivery.</CardDescription>
          </div>
