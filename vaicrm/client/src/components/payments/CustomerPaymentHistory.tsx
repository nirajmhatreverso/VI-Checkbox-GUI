// src/components/customer-payments/CustomerPaymentHistory.tsx
...
import { Receipt, CheckCircle, Clock, X, Info, SlidersHorizontal, Calendar as CalendarIcon, FileText, Loader2, Printer } from "lucide-react";
...
export default function CustomerPaymentHistory() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["customer-hw-payments-history", filters, pageIndex, pageSize],
    enabled: !!user,
    queryFn: () =>
  ...
  const openReceiptDialog = async (transId: string) => {
    ...
    try {
      const res = await apiRequest('/customer-payments/search', 'POST', { 
        transId, 
  ...
  const handleReset = () => {
    setFilters({ transId: "", sapBpId: "", payMode: "", payType: "", collectionCenter: "", fromDate: initialFrom, toDate: initialTo });
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable<PaymentDetail>
        ...
        subtitle="All customer hardware payment transactions"
      ...
      />
      {isError && <p className="text-xs text-red-600">Failed to load payment history.</p>}
...
function AdvancedFiltersComponent({ advFilters, setAdvFilters, setFilters, initialFrom, initialTo, isAdmin }: {
  ...
  useEffect(() => {
    const nextFilters: HistoryFilters = { transId: "", sapBpId: "", payMode: "", payType: "", collectionCenter: "", fromDate: initialFrom, toDate: initialTo };
    ...
    <div className="space-y-3 border p-3 rounded-md">
      ...
      <div className="space-y-2">
        ...
        {advFilters.map((af: AdvancedFilter) => {
          ...
          if (!fieldMeta) return null; // Skip if invalid
          ...
          return (
            <div key={af.id} className="grid grid-cols-12 gap-2 items-center">
              <div className="col-span-3">
                <Select 
                    value={af.field} 
                    onValueChange={(v: FilterFieldKey) => setAdvFilters((p: AdvancedFilter[]) => p.map(x => x.id === af.id ? { 
                        ...x, 
                        field: v, 
                        value: "", 
