...
export default function HardwarePaymentReversalPage() {
    ...
    const reversalMutation = useMutation({
        mutationFn: async (payload: ReversalPayload) => {
            // Assuming endpoint pattern for hardware payment reversal
            return await apiRequest("/customer-payments/reversal", "POST", payload);
        },
        ...
        <div className="p-4 sm:p-6 w-full">
            <div className="bg-gradient-to-r from-azam-blue to-blue-800 text-white p-4 rounded-lg mb-4">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
                    <div>
                        <h1 className="text-xl font-bold">Hardware Payment Reversal</h1>
                        <p className="text-blue-100 text-[11px] md:text-xs leading-tight mt-0.5 sm:block">
                            Reverse a customer hardware payment
                        </p>
                    </div>
                </div>
            </div>
